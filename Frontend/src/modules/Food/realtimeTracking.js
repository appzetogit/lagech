...
export function subscribeOrderTracking(orderId, onChange, onError) {
  ...
  if (!ensureFirebaseInitialized({ enableAuth: true, enableGoogleProvider: false, enableRealtimeDb: true })) return () => {};
...
export async function writeDeliveryLocation({
  ...
  if (!ensureFirebaseInitialized({ enableAuth: true, enableGoogleProvider: false, enableRealtimeDb: true })) return false;
  const payload = {
